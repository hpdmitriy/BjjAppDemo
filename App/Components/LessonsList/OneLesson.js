...
class OneLesson extends Component {
  ...
  handleLessonCreate = (restrictContent, trainingStatus, trainingId = 0, lessonName) => {
    ...
    if(trainingId !== 0) {
      return this.props.navigate('OneTrainingScreen', {id: trainingId, title: lessonName})
    }
      ...
      return (
        <ScrollView style={[baseStyles.container, {marginBottom: 40}]}>
          ...
          <View style={[styles.lessonAttributes, {paddingHorizontal: 15, paddingTop: 15}]}>
            ...
            {
              user.userProfile.role === 'Student' ?   <Label
              text={training.replace('_', ' ')}
              type={training}
              onPress={() => this.handleLessonCreate(restrictContent, training, trainingId, data.title)}
            /> : null
            }
